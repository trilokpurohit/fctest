...
public class Common {
		...
		while (count < 4) {
			try {
				...
				if (locator.startsWith("id=")) {
					...
					locator = locator.substring(6); // remove "class=" from
													// locator
					try {
	...
	/**
	 * This method is used to get all the values from the dynamic list.
	 * 
	 * @param p_Element Element from which we can get the values(e.g.
	 *                  <ul>
	 *                  )
	 * @param p_HtmlTag tag from which actual value resides. e.g. <span>,
	 *                  <h1>,<a>
	 * @return Return an array list containing all the values from given list.
	 */
	public static ArrayList<String> getAllValuesFromDynamicList(WebElement p_Element, String p_HtmlTag) {
		List<WebElement> result = p_Element.findElements(By.tagName(p_HtmlTag));
		ArrayList<String> mList = new ArrayList<String>();
		for (WebElement eleLi : result) {

			mList.add(eleLi.getText());
		}
		return mList;
	}

	/**
	 * This method is used to find specific value from dynamic list.
	 * 
	 * @param p_Element Element from which we can get the values(e.g.
	 *                  <ul>
	 *                  )
	 * @param p_Value   Value to search from the list.
	 * @param p_HtmlTag tag from which actual value resides. e.g. <span>,
	 *                  <h1>,<a>
	 * @return Returns a found value empty otherwise.
	 */
	public static String getValueFromDynamicList(WebElement p_Element, String p_Value, String p_HtmlTag) {
		List<WebElement> result = p_Element.findElements(By.tagName(p_HtmlTag));
		String textFound = "";
		for (WebElement eleLi : result) {

			String value = eleLi.getText();
			if (value.trim().equalsIgnoreCase(p_Value)) {
				textFound = value;
				break;
			}
		}
	...
	public static WebElement getElementFromDynamicList(WebElement p_Element, String p_Value, String p_HtmlTag) {
		List<WebElement> result = p_Element.findElements(By.tagName(p_HtmlTag));
		WebElement ele = null;
		for (WebElement eleLi : result) {

			String value = eleLi.getText();
			log("value: "+value+" p: "+p_Value);
			if (value.trim().equalsIgnoreCase(p_Value)) {
				ele = eleLi;
				log("found value: "+value+" p: "+p_Value);
	...
	public static WebElement getElementFromDynamicList(WebElement p_Element, String p_Value, String p_HtmlTag,
			String p_NestedHtmlTag) {
		List<WebElement> result = p_Element.findElements(By.tagName(p_HtmlTag));
		WebElement ele = null;
		for (WebElement webElement : result) {
			WebElement eleLi = webElement.findElement(By.tagName(p_NestedHtmlTag));

			if (eleLi.getText().trim().equalsIgnoreCase(p_Value)) {
				ele = eleLi;
				break;
	...
	public static void selectMultiPleValuesFromDD(WebDriver p_Driver, ArrayList<String> p_ListOfValues,
			WebElement p_Element) {
		final Select dropdown = new Select(p_Element);
		final List<WebElement> options = dropdown.getOptions();
		final Actions builder = new Actions(p_Driver);
		final boolean isMultiple = dropdown.isMultiple();
		if (isMultiple) {
			dropdown.deselectAll();
		}
		builder.keyDown(Keys.CONTROL);
		for (String textOption : p_ListOfValues) {
